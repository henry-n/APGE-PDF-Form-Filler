...
import { scheduleA, scheduleB } from "./apgeFieldMap";
import { clean, formatContractPrice, formatDateForPdf } from "./formatPdfValues";
import type { ApgeFormData, PdfField, RowColumn } from "./types";

const TEMPLATE_URL = new URL("./templates/APGE Template Full.pdf", window.location.href);

function getPage(pdfDoc: PDFDocument, field: PdfField): PDFPage {
  const page = pdfDoc.getPages()[field.page];
  if (!page) throw new Error(`PDF template is missing page ${field.page + 1}.`);
  return page;
}

function wrapText(text: unknown, font: PDFFont, size: number, maxWidth: number, maxLines = 2): string[] {
...
function drawText(pdfDoc: PDFDocument, font: PDFFont, field: PdfField, value: unknown): void {
...
function drawCheck(pdfDoc: PDFDocument, font: PDFFont, field: PdfField, shouldCheck: boolean): void {
...
function drawRowText(page: PDFPage, font: PDFFont, column: RowColumn, y: number, value: unknown): void {
  const text = clean(value);
  if (!text) return;

  const size = column.size ?? 7;
  const lines = wrapText(text, font, size, column.maxWidth ?? 100, 3);
  lines.forEach((line, index) => {
    page.drawText(line, {
      x: column.x,
      y: y - index * (size + 2),
...
function drawRowCheck(page: PDFPage, font: PDFFont, column: RowColumn, y: number, shouldCheck: boolean): void {
  if (!shouldCheck) return;
  page.drawText("X", {
    x: column.x,
    y,
...
export async function fillApgePdf(formData: ApgeFormData): Promise<Uint8Array> {
  ...
  if (!pageB) throw new Error("PDF template is missing Schedule B page.");

  const rows = formData.serviceLocations.slice(0, scheduleB.maxRows);
  ...
  rows.forEach((location, index) => {
    ...
    drawRowCheck(pageB, font, cols.taxExemptCheck, y, location.taxExempt);
